...
export default function AddEntryScreen({ navigation }) {
  ...
  }, [accountId, amount, categoryId, isLoadingOptions, isSaving, source, transactionDate]);
  ...
  const buildDescription = () => {
    ...
    if (reference.trim()) {
      parts.push(`Reference: ${reference.trim()}`);
    }
  ...
  const handleSave = async () => {
    ...
    if (
      ...
      showError('Tous les champs obligatoires doivent etre renseignes.');
      return;
    }
    ...
    try {
      await api.send(payload, 'transactions');

      Alert.alert('Entree enregistree', 'L entree a ete ajoutee a la petite caisse.', [
        { text: 'Rester ici' },
  ...
  const selectorItems = selectorType === 'account' ? accounts : categories;
  const selectorTitle = selectorType === 'account' ? 'Choisir un compte' : 'Choisir une categorie';
  ...
  return (
    <View style={[styles.safeArea, { paddingTop: insets.top }]}>
                ...
                selectorItems.map((item) => (
                  ...
                  >
                    <View>
                      ...
                      <Text style={styles.optionMeta}>ID: {item?.id}</Text>
                    </View>
                  </TouchableOpacity>
                ))
              )}
      ...
      >
        ...
        <Text style={styles.title}>Ajouter une entree</Text>
        <Text style={styles.subtitle}>
          Renseignez les informations necessaires pour creer une transaction d entree.
        </Text>

        <View style={styles.heroCard}>
          <Text style={styles.heroLabel}>Type fixe</Text>
          <Text style={styles.heroText}>
            Cette operation sera enregistree comme une entree (`income`) dans la caisse.
          </Text>
        </View>
        ...
        <View style={styles.formCard}>
          ...
          <TextInput
            ...
            placeholder="Ex. Versement, recette, remboursement"
          ...
          <TextInput
            ...
            placeholder="Numero de piece ou note interne"
            placeholderTextColor="#7A8A90"
            style={styles.input}
            value={reference}
          />
          ...
          <TextInput
            ...
            placeholder="Contexte de l entree"
            placeholderTextColor="#7A8A90"
          ...
          >
            <Text style={styles.buttonText}>
              {isSaving ? 'Enregistrement...' : 'Enregistrer l entree'}
            </Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
